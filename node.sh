...
install_nvm() {
    ...
    # Reload NVM in the current shell
    load_nvm
    # Verify NVM installation
    if ! command -v nvm &> /dev/null; then
        echo "Error: NVM installation failed. Exiting..."
        exit 1
    fi
}
...
install_node_npm() {
    ...
    if ! command -v npm &> /dev/null; then
        echo "npm not found. Reinstalling Node.js to include npm..."
        nvm install node
    fi
...
check_commands() {
    echo "Checking if nvm, node, and npm are accessible..."
    if ! command -v nvm &> /dev/null; then
        echo "Error: nvm is not accessible. Please check the installation."
        exit 1
    fi
    if ! command -v node &> /dev/null; then
        echo "Error: node is not accessible. Please check the installation."
        exit 1
    fi
    if ! command -v npm &> /dev/null; then
        echo "Error: npm is not accessible. Please check the installation."
        exit 1
    fi
